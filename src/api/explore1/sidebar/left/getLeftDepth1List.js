...
async function getLeftDepth1List(depth, filter, currentId) {
  const bookUrl = `/book/${depth}/${currentId}`;
  const authorUrl = `/author/${depth}/${currentId}`;
  const response = axios({
    url: filter === 'book' ? bookUrl : authorUrl,
    method: 'get',
    data: {
      depth: depth,
      currentId: currentId,
...
export default getLeftDepth1List;
