...
async function getDepth2List(depth, currentId) {
  ...
  const depth2DatasA = JSON.parse(JSON.stringify(leftDatasDepth2A));
  const depth2DatasB = JSON.parse(JSON.stringify(leftDatasDepth2B));
  const depth2DatasAll = JSON.parse(JSON.stringify(leftDatasDepth2All));
  if (currentId === 'A') return depth2DatasA;
  else if (currentId === 'B') return depth2DatasB;
  return depth2DatasAll;
}

export default getDepth2List;
